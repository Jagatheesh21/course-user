...
class Files {
	...
	public static function File_upload($uploadedFile, $dir) {
		$newName = self::generateNewFileName($uploadedFile->getClientOriginalName());

		if ($dir == 'bill_file' || $dir == 'invoice_file') {
			...
			$tempPath = public_path($dir . '/' . $newName);
		} else {
			...
			$directory = 'user-uploads/' . $dir;
			if (is_dir(public_path($directory)) == false) {

				mkdir(public_path($directory));
			}

			$tempPath = public_path($directory . '/' . $newName);
		}
	...
	public static function File_upload_with_name($uploadedFile,$name) {
		$directory = 'uploads';
		if (is_dir(public_path($directory)) == false) {
